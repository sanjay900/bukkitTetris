...
import com.sanjay900.tetris.SETTINGS.Utils;
...
public class GameLoop implements Runnable
{
	...
	public void run()
	{
		...
		{
			this.timeoutTicks -= 1;
			Utils.sendActionBar(game.player, ChatColor.BOLD+"You will be sent to the lobby in "+ChatColor.RED+ChatColor.BOLD+""+timeoutTicks/20+ChatColor.BOLD+" seconds!");
			if (this.timeoutTicks == -1)
			{
				Core.getInstance().getLocalPlayer(this.game.getPlayer()).setGame(null);
		...
		{ 
			if (!this.midiPlaying) {
				...
				File[] trialPaths = {
						new File(Core.getInstance().getDataFolder().getPath()+"/midi/", midiName)
				};
				File file = null;
				...
				if(file != null && file.exists()) {
					try {
						...
						Bukkit.getScheduler().runTaskAsynchronously(Core.getInstance(), p);
